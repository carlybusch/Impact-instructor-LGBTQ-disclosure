...
bar_plot <- means |>
  ...
  geom_errorbarh(aes(xmax = mean + sd, xmin = mean - sd), height = 0.2) +
  geom_text(aes(x = mean + sd + 0.15, label = round(mean, 1)), vjust = 0.5, size = 3, color = "black") +
  labs(x = "Mean ± SD", y = "") +
  scale_x_continuous(
    limits = c(0, 5), 
...
fig1 <- ggpubr::ggarrange(bar_plot, fig1b, fig1c, fig1d, fig1e, fig1f,
                          labels = c("A.", "B.", "C.",
                                     "D.", "E.", "F."), 
...
fig2 <- mod_out |>
  ...
  geom_point(aes(x = est, shape = psig), size = 3.5, position = position_dodge(0.75)) +
  geom_errorbarh(aes(xmin = est - 1.96*se, xmax = est + 1.96*se, linewidth = psig), 
...
fig3 <- mod_out |>
  ...
  mutate(outcome = forcats::fct_relevel(outcome, "sci_morale_mean",
                                        ...
                                        "connect_mean")) %>%
  ggplot(aes(y = outcome)) +
  geom_point(aes(x = est), size = 3.5, shape = 23) +
